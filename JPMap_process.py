...
def create_league(songs, start, last, division, superuser):
    ...
    os.environ.setdefault("DJANGO_SETTINGS_MODULE", "jbsl3.settings")
    django.setup()
    from app.models import League, Playlist, Song
    ...
    from app.utils import char_dict_inv, diff_label_inv
    ...
    if len(songs) > 0:
        ...
        for song in songs:
            ...
            if not Song.objects.filter(
                hash=song.hash, diff=song.diff, char=song.char
            ).exists():
                lid = get_scoresaber_lid(song.hash, gameMode, dif_num)
                if lid == None:
                    ...
                    if bid == -1:
                        ...
                        continue
                    print(bid)
                    new_song = create_song_by_beatleader(
                        hash=hash, char=char, dif=dif, bid=bid
                    )
                    print(new_song)
                    if new_song == None:
                        continue
                ...
                print(lid)
                if lid == None:
                    new_song = Song.objects.filter(
                        hash=song.hash, char=song.char, diff=song.diff
                    ).first()
                else:
                    new_song = create_song_by_scoresaber(
                        song.hash, dif_num, song.char, lid
                    )
            print(new_song)
            if new_song == None:
                continue
...
def monthly():
    ...
    for song in JPMap.objects.filter(
        createdAt__gte=start, createdAt__lte=start_end
    ).order_by("-nps"):
...
def add_songs_to_playlist(playlist, songs):
    ...
    for song in songs:
        ...
        if not Song.objects.filter(
            ...
            lid = get_scoresaber_lid(song.hash, gameMode, diff_num)
            if lid is None:
                continue
        else:
            lid = Song.objects.get(hash=song.hash, diff=song.diff, char=song.char).lid
        new_song = create_song_by_scoresaber(song.hash, diff_num, song.char, lid)
        if new_song is None:
            hash = song.hash
            dif = song.diff
            char = song.char
...
def latest():
    from app.models import JPMap, Player, Playlist, Song, SongInfo
    from app.operations import (
        char_dict_inv,
        create_song_by_beatleader,
        create_song_by_scoresaber,
    ...
    for player in Player.objects.filter(mapper__gt=0).order_by("mapper_name"):
        ...
        for jmap in JPMap.objects.filter(uploader=player).order_by(
            "-createdAt", "-char", "-nps"
        ):
            print(jmap)
            ...
            if song == None:
                lid = get_scoresaber_lid(jmap.hash, gameMode, diff_num)
                if lid is None:
                    print("no lid detected")
                    # try beatleader
                    bid = get_beatleader_bid(hash, dif, char)
