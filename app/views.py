...
from django.db.models import Count, ExpressionWrapper, F, FloatField, Max, Prefetch
...
from .operations import (
    ...
    calculate_scoredrank_LBs,
    check_membership_and_ownership,
    create_song_by_beatleader,
    create_song_by_scoresaber,
    get_headline_and_league_context,
    get_join_state,
    get_scoresaber_lid,
    make_sorted_playlist,
    score_to_headline,
    top_score_registration,
)
from .post_handlers import post_index, post_leaderboard, post_userpage
from .utils import (
    ComparedScore,
    Results,
    char_dict_inv,
...
def index(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        post = request.POST
        response = post_index(post, player)
        if response:
            return response
...
def userpage(request: HttpRequest, sid=None) -> HttpResponse:
    context = {}
    user = request.user
    if user.is_authenticated:
        social = SocialAccount.objects.get(user=user)
    ...
    if request.method == "POST":
        post = request.POST
        response = post_userpage(post, user, player, context)
        if response:
            return response

    eyebeam_count = Player.objects.filter(rival=player).count()
    top10_scores = (
        Score.objects.filter(player=player, league__name="Top10")
        .order_by("-rawPP")
    ...
    player_stats = calculate_player_stats(player)
...
def activate_process(request):
    ...
    if request.method == "POST":
        ...
        if res["country"] != "JP":
            context[
                "error"
            ] = "日本人以外のプレイヤーは登録できません。Sorry, Only Japanese player can be registered."
            return render(request, "activation.html", context)
        if Player.objects.filter(sid=sid).exists():
            context[
                "error"
            ] = "すでにこのスコアセイバーIDで登録しているユーザーがいます。もし、自分のアカウントを他のユーザーによって使われている場合、管理者（hibit）までご連絡ください。"
            return render(request, "activation.html", context)
        name = res["name"]
        ...
        Headline.objects.create(player=player, text=text, time=datetime.now())
...
def playlists(request, page=1):
    ...
    if user.is_authenticated:
        ...
        playlists = (
            playlists.order_by("-pk")
            .filter(Q(isHidden=False) | Q(editor=user.player) | Q(CoEditor=user.player))
            .distinct()[start:end]
        )
    # ユーザーが認証されていない場合の処理
    else:
...
def playlist(request, pk):
    ...
    if playlist.isHidden:
        if not user.is_authenticated:
            ...
            if not (
                playlist.editor == user.player or user.player in playlist.CoEditor.all()
            ):
                return redirect("app:index")
    ...
    if user.is_authenticated:
        if playlist.editor == user.player:
            ...
            for i, song in enumerate(playlist.sorted_songs):
                # print(song)
                songInfo, _ = SongInfo.objects.get_or_create(
    ...
    if user.is_authenticated:
        ...
        context["social"] = social
        isEditor = (
            user.player == playlist.editor or user.player in playlist.CoEditor.all()
        )
        ...
        "songs": [
            {
                ...
                "hash": song.hash,
                "difficulties": [{"characteristic": song.char, "name": song.diff}],
            }
...
def leaderboard(request, pk):
    ...
    if player:
        ...
        is_prohibited = any(
            user_league in prohibited_leagues for user_league in user_leagues
        )
    #
    ...
    join_state = get_join_state(
        user, is_member, league, player, is_close, is_prohibited
    )
    ...
    context.update(
        {
            ...
            "join_comment": join_comment[join_state],
            "edit_state": is_owner and league.isLive,
            "isOwner": is_owner,
            "isMember": is_member,
            "end_str": end_str,
            "close_str": close_str,
    ...
    if request.method == "POST":
        post = request.POST
        response = post_leaderboard(post, league)
        if response:
            return response
...
def headlines(request, page=1):
    ...
    league_prefetch = Prefetch(
        "score", queryset=Score.objects.select_related("league", "song")
    )
    headlines = Headline.objects.prefetch_related("player", league_prefetch).order_by(
        "-time"
    )[start:end]
...
def players(request, sort="borderPP", page=1):
    ...
    if user.is_authenticated:
        ...
        if player.isActivated:
            ...
            context["invitations"] = invitations
    active_players = (
        Player.objects.filter(isActivated=True)
        .prefetch_related("badges")  # related_nameに指定された値を使用
...
def bsr_checker(request):
    ...
    if request.method == "POST":
        ...
        results.append(
            f"マッチするプレイヤー：{player.name}",
            f"https://jbsl-web.herokuapp.com/userpage/{player.sid}",
        )
        results.append(
            f"Score Saber : {player.sid}", f"https://scoresaber.com/u/{player.sid}"
        )
        context["results"] = results
        ...
        for i, diff in enumerate(diffs):
            ...
            lid = res["id"]
            results.append(
                f"diff {i+1} {chara} {label} リーダーボード ID : {lid}",
...
def league_edit(request, pk):
    ...
    not_invite_players = (
        Player.objects.exclude(league=league)
        .exclude(invite=league)
...
def short_leaderboard(request, pk=0):
    ...
    for song in songs:
        query = (
            Score.objects.filter(song=song, league=league, player__league=league)
            .order_by("-score")
            .first()
        )
    ...
    context["participants"] = Participant.objects.filter(league=league).order_by(
        "-count_pos", "-count_acc"
    )
    durtaion = time() - duration_start
    context["duration"] = durtaion * 1000
...
def match(request, pk=1):
    ...
    if request.method == "POST":
        ...
        if "league" in post:
            ...
            participants = Participant.objects.filter(league=league).order_by(
                "-count_pos", "-count_acc"
            )
    ...
    # sorted_player
    participants = Participant.objects.filter(league=match.league).order_by(
        "-count_pos", "-count_acc"
    )[: match.league.border_line]
