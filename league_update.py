...
def league_end_process(league, time_count) -> str:
    ...
    Headline.objects.create(
        player=None,
        time=record_time + timedelta(seconds=time_count),
...
def notify_grand_slam(player, league):
...
def update_league_ranking(league):
    ...
    for i, player in enumerate(players[:3]):
        if i == 0 and league.first != player:
            ...
            notify_grand_slam(player, league)
            league.first = player
...
def update_score_by_beatleader(league, song, player):
    ...
    acc = (
        float(res["accuracy"]) * 100 * int(res["modifiedScore"]) / int(res["baseScore"])
    )
    print("       ", score, acc)
    rawPP = 0
